...
export class ProfileController {
   ...
   */
  static async upsertUserProfile(req: Request, res: Response) {
    const {
      id,
      email,
      name,
    ...
    let profile = null;
    if (id) {
      profile = await prisma.userProfile.findUnique({ where: { id } });
    }
    if (!profile && email) {
    ...
    if (parsedWeight !== undefined && !isNaN(parsedWeight)) updateData.weight = parsedWeight;
    if (bloodType !== undefined) updateData.bloodType = bloodType;
    if (activityLevel !== undefined) updateData.activityLevel = activityLevel;
    if (budgetRange !== undefined) updateData.budgetRange = budgetRange;
    if (parsedMedicalConditions !== undefined) updateData.medicalConditions = parsedMedicalConditions;
    if (parsedHealthGoals !== undefined) updateData.healthGoals = parsedHealthGoals;
    if (parsedAllergies !== undefined) updateData.allergies = parsedAllergies;
    if (parsedAvailableFoods !== undefined) updateData.availableFoods = parsedAvailableFoods;
