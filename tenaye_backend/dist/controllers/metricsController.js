...
class MetricsController {
     ...
     */
    static async logMetric(req, res) {
    ...
    static async getMetricsHistory(req, res) {
        ...
        const grouped = measurements.reduce((acc, item) => {
    ...
    static async updateMetric(req, res) {
        ...
        const measurement = await database_1.prisma.healthMeasurement.findUnique({
            where: { id },
        });
        if (!measurement) {
            res.status(404).json({
        ...
        const updated = await database_1.prisma.healthMeasurement.update({
            where: { id },
            data: {
                value: String(value),
    ...
    static async deleteMetric(req, res) {
        ...
        await database_1.prisma.healthMeasurement.delete({
            where: { id },
        });
        res.status(200).json({
            success: true,
